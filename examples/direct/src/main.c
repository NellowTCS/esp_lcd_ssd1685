...
/* Source-line offset on this panel variant (adjust if first column is off) */
 ...
 * Row stride = (width + 7) / 8 bytes (NO extra padding needed by the driver). */
...
/* Fill a rectangular region of a 1bpp bitmap */
...
/* Draw a 1-pixel-wide rectangle border */
...
/* 8x8 font, ASCII 32-127 subset (each char = 8 bytes, 1 byte per row)
 * Sourced from public-domain 8x8 bitmap font.                            */
static const uint8_t font8x8[96][8] = {
    ...
    {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}, /* DEL (used as filled block) */
};

/* Draw a character into the 1bpp bitmap using the 8x8 font.
 * x, y = top-left of the glyph in the framebuffer.                       */
...
/* Scene 1: full white then full black, basic sanity check */
static void demo_solid(esp_lcd_panel_handle_t panel, int w, int h)
{
    ...
    esp_lcd_ssd1685_clear(panel, 0xFF);          /* also clears RED plane */
...
/* Scene 2: 16-pixel-wide vertical stripes */
static void demo_stripes(esp_lcd_panel_handle_t panel, int w, int h)
{
    ...
    memset(buf, 0x00, bytes);   /* start black */

    for (int x = 0; x < w; x++) {
        int white = ((x / 16) & 1) ? 1 : 0;
        for (int y = 0; y < h; y++) {
            bmp_set_pixel(buf, stride, x, y, white);
        }
    }
...
/* Scene 3: checkerboard (8x8 squares) */
...
static void demo_text(esp_lcd_panel_handle_t panel, int w, int h)
{
    ...
    /* Outer border */
    bmp_draw_rect(buf, w, h, 0, 0, w, h, 0);

    /* Title box */
    bmp_fill_rect(buf, w, h, 2, 2, w - 4, 18, 0);
    bmp_draw_string(buf, w, h, 4, 5, "SSD1685 Driver Demo", 1);

    /* Info lines */
    char line[64];
    snprintf(line, sizeof(line), "Width : %d px", w);
    bmp_draw_string(buf, w, h, 4, 26, line, 0);

    snprintf(line, sizeof(line), "Height: %d px", h);
    bmp_draw_string(buf, w, h, 4, 36, line, 0);

    bmp_draw_string(buf, w, h, 4, 46, "1 bpp  MSB-first  no padding", 0);
    bmp_draw_string(buf, w, h, 4, 56, "esp_lcd_ssd1685 driver OK!", 0);

    /* A few filled squares as a pixel-accuracy check */
    for (int i = 0; i < 8; i++) {
        bmp_fill_rect(buf, w, h, 4 + i * 20, 70, 16, 16, (i & 1) ? 1 : 0);
        if (!(i & 1))
            bmp_draw_rect(buf, w, h, 4 + i * 20, 70, 16, 16, 0);
    }

    bmp_draw_string(buf, w, h, 4, 92, "0123456789 ABCDEFGHIJKLMNOP", 0);
    bmp_draw_string(buf, w, h, 4, 102, "abcdefghijklmnopqrstuvwxyz", 0);
    bmp_draw_string(buf, w, h, 4, 112, "!@#$%^&*()-=[]{}\\|;':\",./<>?", 0);

    /* Diagonal line */
    {
        int n = (w < h - 130) ? w : h - 130;
        for (int i = 0; i < n; i++)
            bmp_set_pixel(buf, stride, i, 130 + i, 0);
    }
...
/* Scene 5: partial-area update, update only a sub-rectangle */
static void demo_partial(esp_lcd_panel_handle_t panel, int w, int h)
{
    ESP_LOGI(TAG, "=== Scene 5: partial area update ===");

    /* First fill the whole screen white */
    esp_lcd_ssd1685_clear(panel, 0xFF);
    vTaskDelay(pdMS_TO_TICKS(2000));

    /* Now draw a black 64x64 square in the centre using a sub-bitmap */
    int px = (w - 64) / 2;
    int py = (h - 64) / 2;
    int pw = 64, ph = 64;
    int pstride = (pw + 7) / 8;
    ...
    memset(pbuf, 0x00, (size_t)pstride * ph); /* all black */

    /* Write border white so we can see alignment */
    bmp_draw_rect(pbuf, pw, ph, 0, 0, pw, ph, 1);

    /* Use non_copy_mode variant: draw then manually refresh */
...
void app_main(void)
{
    ...
    spi_bus_config_t bus_cfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = (PANEL_WIDTH * PANEL_HEIGHT) / 8 + 16,
    };
    ...
    /* 6. Apply gap correction for this panel variant */
    if (PANEL_GAP_X != 0 || PANEL_GAP_Y != 0) {
        ESP_ERROR_CHECK(esp_lcd_panel_set_gap(panel, PANEL_GAP_X, PANEL_GAP_Y));
    }

    /* Logical dimensions after any rotation applied by the panel */
    int logical_w = PANEL_WIDTH;
    int logical_h = PANEL_HEIGHT;

    ESP_LOGI(TAG, "Panel ready  logical %dx%d", logical_w, logical_h);
    ...
    /* Run demo scenes in a loop */
    while (true) {
        demo_solid  (panel, logical_w, logical_h);
        demo_stripes(panel, logical_w, logical_h);
        demo_checker(panel, logical_w, logical_h);
        demo_text   (panel, logical_w, logical_h);
        demo_partial(panel, logical_w, logical_h);
    }
}
