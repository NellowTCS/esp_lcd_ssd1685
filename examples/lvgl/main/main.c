...
/*
 * Derived LVGL dimensions: rotation + gap.
 *
 * PANEL_GAP_X offsets the RAM window in the physical X (source) direction.
 * After rotation that maps to different LVGL axes:
 *   rot 0/2 (portrait/180°):    gap_x shrinks LVGL width
 *   rot 1/3 (landscape CW/CCW): gap_x shrinks LVGL height
 *                               (physical X becomes LVGL Y after swap_xy)
 *
 * Bitmaps wider/taller than LVGL_WIDTH/LVGL_HEIGHT overflow into the next
 * RAM row, shifting every subsequent row and producing diagonal skew.
 */
#if PANEL_ROTATION == 1 || PANEL_ROTATION == 3
#define LVGL_WIDTH (PANEL_HEIGHT - PANEL_GAP_Y)
#define LVGL_HEIGHT (PANEL_WIDTH - PANEL_GAP_X)
#else
#define LVGL_WIDTH (PANEL_WIDTH - PANEL_GAP_X)
#define LVGL_HEIGHT (PANEL_HEIGHT - PANEL_GAP_Y)
#endif
    ...
    void app_main(void)
{
    ...
    /* Rotation (hardware: swap_xy + mirror flags)
    ...
    /* Allocate EPD repack buffer (1-byte-aligned rows) */
    ...
    if (!buf1)
        buf1 = heap_caps_malloc(lvgl_buf_sz, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
    uint8_t *buf2 = heap_caps_malloc(lvgl_buf_sz, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!buf2)
        buf2 = heap_caps_malloc(lvgl_buf_sz, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
